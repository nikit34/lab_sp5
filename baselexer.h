...
class tBaseLexer {
    std::ifstream fsource; // поток ввода из файла
    int line; // счетчик строк
    char buf[1000]; // буфер строки
    static const size_t bufsize = 1000;
    char *start; // указатель начала лексемы в буфере
    char *end; // указатель конца лексемы в буфере
...
public:
    ...
    // функции-члены
    bool Begin(const char *filename); // подготовка к работе с файлом
    void End() { fsource.close(); } // завершение работы с файлом

    std::string GetToken(); // извлекает из текста очередной токен
