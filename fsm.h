...
public:
    ...
    void final(tState st); // включает одно состояние во множество заключительных
    int apply(const tSymbol *input); // применяет автомат к входной цепочке

    size_t size() const { return table.size(); } // выдает размер (количество состояний) автомата
...
private:
    ...
    tStateSet finals; // множество заключительных состояний
};
...
inline void tFSM::add(tState from, tSymbol c, tState to) {
    ...
    if (sz > table.size())table.resize(sz); // увеличивает размер вектора до sz
    table[from][c] = to; // два перегруженных оператора []: один для vector, другой для map.
}

inline void tFSM::final(tState st) { finals.insert(st); }

inline int tFSM::apply(const tSymbol *input) {
    if (table.empty()) return 0;// пустая таблица состояний
    tState state = 0; //начальное состояние
    int accepted = 0;

    // цикл прохода по входной цепочке
    while (*input) {
        tTransMap::iterator iter;// итератор контейнера map
        tTransMap &trans = table[state];// ссылка на таблицу переходов из состояния state
